...
function getExportDefaultLocale(exportManifest) {
    if (typeof exportManifest?.defaultLocale === 'string' && exportManifest.defaultLocale.trim()) {
        return exportManifest.defaultLocale.trim();
    }

    const defaultLocale = exportManifest?.locales?.find((locale) => locale?.isDefault && locale?.code);
    return defaultLocale?.code || null;
}

function shouldPublishDocument(model, payload) {
    return Boolean(model?.options?.draftAndPublish !== false && payload?.publishedAt);
}

function getDocumentStatus(model, payload) {
    if (model?.options?.draftAndPublish === false) {
        return 'published';
    }

    return shouldPublishDocument(model, payload) ? 'published' : 'draft';
}

async function findExistingDocumentVariant(strapi, uid, documentId, locale) {
    if (!strapi.documents || !documentId) return null;
    ...
    return await strapi.documents(uid).findFirst({ ...query, status: 'draft' })
        || await strapi.documents(uid).findFirst({ ...query, status: 'published' });
}

async function findSingleTypeDocument(strapi, uid, locale) {
    if (!strapi.documents) return null;

    const localizedQuery = locale ? { locale } : {};

    return await strapi.documents(uid).findFirst({ ...localizedQuery, status: 'draft' })
        || await strapi.documents(uid).findFirst({ ...localizedQuery, status: 'published' })
        || await strapi.documents(uid).findFirst({ status: 'draft' })
        || await strapi.documents(uid).findFirst({ status: 'published' });
}

async function publishDocumentVariant(strapi, uid, documentId, locale) {
    if (!strapi.documents || !documentId) return;
    await strapi.documents(uid).publish({ documentId, locale });
}

async function syncDefaultLocale(strapi, exportManifest, options = {}) {
    const sourceDefaultLocale = getExportDefaultLocale(exportManifest);
    if (!sourceDefaultLocale) return;

    const localeService = strapi.plugin('i18n').service('locales');
    const targetDefaultLocale = await localeService.getDefaultLocale();

    if (targetDefaultLocale === sourceDefaultLocale) {
        console.log(`    - Default locale already matches source: ${sourceDefaultLocale}`);
        return;
    }

    const sourceLocaleExists = await localeService.findByCode(sourceDefaultLocale);
    if (!sourceLocaleExists) {
        console.warn(`    ! Cannot set default locale to ${sourceDefaultLocale}: locale is missing on target`);
        return;
    }

    if (options.dryRun) {
        console.log(`    [DRY-RUN] Would change default locale from ${targetDefaultLocale || '(unset)'} to ${sourceDefaultLocale}`);
        return;
    }

    await localeService.setDefaultLocale({ code: sourceDefaultLocale });
    console.log(`    - Updated default locale: ${targetDefaultLocale || '(unset)'} -> ${sourceDefaultLocale}`);
}
...
async function ensureLocaleExists(strapi, localeObj) {
    ...
    try {
        const localeCode = normalizeLocaleValue(localeObj.code);
        const localeName = normalizeLocaleValue(localeObj.name);
        const existingLocales = await strapi.db.query('plugin::i18n.locale').findMany();

        const existing = existingLocales.find(locale => {
            const codeMatches = normalizeLocaleValue(locale.code) === localeCode;
            const nameMatches = localeName && normalizeLocaleValue(locale.name) === localeName;
...
async function runImport(userInputPath, options = {}) {
  ...
  if (exportManifest.locales && exportManifest.locales.length > 0) {
      ...
      await syncDefaultLocale(strapi, exportManifest, options);
  }
  ...
  for (const uid of uids) {
      ...
      for (const item of items) {
          ...
          try {
              ...
              if (documentId) {
                   if (strapi.documents) {
                       existing = await findExistingDocumentVariant(strapi, uid, documentId, targetLocale);
                   } else {
              ...
              if (existing) {
                  ...
                  if (strapi.documents) {
                      await strapi.documents(uid).update({ 
                          ...
                          status: getDocumentStatus(model, rawPayload)
                      });
                  } else {
          ...
          try {
              ...
              if (strapi.documents) {
                  try {
                      ...
                      // Handle Publishing
                        if (shouldPublishDocument(model, rawPayload)) {
                          if (model.options?.draftAndPublish !== false) {
                              await publishDocumentVariant(strapi, uid, documentId, rawPayload.locale);
      ...
      if (model && model.kind === 'singleType') {
           ...
           if (items && items.length > 0) {
               ...
               for (const item of items) {
                   ...
                   try {
                      const targetLocale = rawPayload.locale;
                      const targetStatus = getDocumentStatus(model, rawPayload);

                      if (strapi.documents) {
                          localEntry = await findSingleTypeDocument(strapi, uid, targetLocale);
                      }
                      ...
                      if (targetDocId) {
                          ...
                          if (strapi.documents) {
                              ...
                              localEntry = await findSingleTypeDocument(strapi, uid, targetLocale);
                              targetDocId = localEntry ? localEntry.documentId : targetDocId;
                          } else {
                      ...
                      if (shouldPublishDocument(model, rawPayload) && targetDocId) {
                           await publishDocumentVariant(strapi, uid, targetDocId, targetLocale);
                      }
